  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post("/api/trips", {
        ...
        startDate: destinations[0].dates.start,
        endDate: destinations[0].dates.end,
      });
      const tripId = data._id;
      const { data: placesData } = await axios.get(
        `/api/trips/places?destination=${destinations[0].city}`
      );
      setPlaces(placesData);

      await axios.post("/api/trips/places", {
        tripId,
        placeId: placesData[0].xid,
      });
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <>
      ...
      <div className="form-container">
        ...
        <button className="add-destination" onClick={addDestination}>
          + Add destination
        </button>
